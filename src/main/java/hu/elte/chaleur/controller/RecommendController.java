...
public class RecommendController {
    ...
    public List<Recipe> recommend(){
        ...
        //Kérem a legutóbbi DailyDifferent átlagot (ez az elmúlt 3 nap összesített fogyasztásainak átlaga)
        ...
        //Eltávolítjuk a listából azokat a recepteket, melyeket fogyasztott a user az utolsó néhány napban.
    ...
    public Double testMe(){
        ...
        Double lastDaysNumber = (double) dailyDifferents.size();

        List<DailyDifferentNutrient> oldValues = dailyDifferentRepository.findById(1).get().getDailyDifferentNutrients();
        List<DailyDifferentNutrient> newValues = dailyDifferentRepository.findById(3).get().getDailyDifferentNutrients();


        Double fejlodes = 0.0;
        for(DailyDifferentNutrient old : oldValues){
            for(DailyDifferentNutrient neVV : newValues){
                if(old.getNutrient().equals(neVV.getNutrient())){
                    if(neVV.getDifferent() > 0){
                        fejlodes += (old.getDifferent() / lastDaysNumber) - old.getDifferent();
                    } else{
                        fejlodes += ((neVV.getDifferent() + old.getDifferent()) / lastDaysNumber) - old.getDifferent();
                    }
                    System.err.println("["+ old.getNutrient().getName()+"]fejlodés: "+fejlodes);
                }
            }
        }
        return fejlodes;
