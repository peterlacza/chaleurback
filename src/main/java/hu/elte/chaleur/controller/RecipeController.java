...
public class RecipeController {
    ...
    //TODO: get my followings latest recipes, mindnekitől a legutolsó feltöltés
    @GetMapping("/latest")
    ...
    public String setDate(){
        for(Recipe recipe : recipeRepository.findAll()){
            if(recipe.getCreatedAt() == null){
                Integer randomDays = (int)(Math.random() * (100 - 10 + 1) + 10);
                Integer randomHours = (int)(Math.random() * (23 - 1 + 1) + 1);

                recipe.setCreatedAt(LocalDateTime.now().minusDays(randomDays).minusHours(randomHours));
                recipeRepository.save(recipe);
            }
        }
        return "ez meg is volna :)";
    }

    @DeleteMapping("/delete")
    public void deleteReciep(@RequestParam(value = "recipeId", required = true) Integer recipeId){
    ...
    public List<Recipe> findFavourites(String userName){
